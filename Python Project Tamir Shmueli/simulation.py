...
if __name__ == '__main__':
    ...
    sim.Config.simulation_steps = 100
    ...
    #sim.population_init()
    arr=[0,0.25,0.5,0.6,0.7,0.8,0.9,0.95,0.99]

    #set lockdown scenario
    
    #set self-isolation scenario
    sim.Config.set_lockdown(lockdown_percentage=0.1,lockdown_compliance=0.1)
    
    if(k==0):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[0],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==1):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[1],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==2):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[2],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==3):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[3],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)

    if(k==4):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[4],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==5):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[5],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==6):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[6],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==7):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[7],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==8):
         sim.Config.set_self_isolation(self_isolate_proportion = arr[8],
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
    if(k==9):
         sim.Config.set_self_isolation(self_isolate_proportion = 1,
                               isolation_bounds = [0.02, 0.02, 0.09, 0.98],
                               traveling_infects=False)
